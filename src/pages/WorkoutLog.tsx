          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex gap-2 p-1 bg-background/50 rounded-lg">
                <button type="button" onClick={() => setType('musculacao')}
                ...
                <button type="button" onClick={() => setType('corrida')}
                  ...
                  <div key={index} className="p-4 bg-background/30 rounded-xl border border-border/50 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="md:col-span-2">
                        <input type="text" placeholder="Nome do exercício" value={ex.name} required
                      ...
                      <div className="flex gap-4">
                        <div className="flex-1">
                          ...
                          <input type="number" min="0" value={ex.sets} required onChange={(e) => handleExerciseChange(index, 'sets', parseInt(e.target.value))} className="w-full px-3 py-2 bg-background border border-border rounded-lg focus:ring-2 focus:ring-primary/50 focus:outline-none" />
                        </div>
                        <div className="flex-1">
                          <label className="text-xs text-muted-foreground">Reps</label>
                          <input type="number" min="0" value={ex.reps} required onChange={(e) => handleExerciseChange(index, 'reps', parseInt(e.target.value))} className="w-full px-3 py-2 bg-background border border-border rounded-lg focus:ring-2 focus:ring-primary/50 focus:outline-none" />
                        </div>
                      </div>
                      <div>
                        <label className="text-xs text-muted-foreground">Peso (kg)</label>
                        <input type="number" step="0.5" min="0" value={ex.weight} required onChange={(e) => handleExerciseChange(index, 'weight', parseFloat(e.target.value))} className="w-full px-3 py-2 bg-background border border-border rounded-lg focus:ring-2 focus:ring-primary/50 focus:outline-none" />
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" step="0.01" min="0" value={distance} required onChange={(e) => setDistance(parseFloat(e.target.value))}
                  ...
                  <div>
                    ...
                    <input type="text" pattern="[0-9]{1,2}:[0-9]{2}:[0-9]{2}" placeholder="00:30:00" value={durationStr} required onChange={(e) => setDurationStr(e.target.value)}
                      className="w-full px-4 py-2 bg-background/50 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" />
                  </div>
